...
#define TEXTURE_PATTERN_DIVIDE_X	(16)		// アニメパターンのテクスチャ内分割数（X)
...
static TUTORIALENEMY	g_Enemy[TUTORIALENEMY_MAX];		// エネミー構造体

static int		g_EnemyCount = TUTORIALENEMY_MAX;

static INTERPOLATION_DATA g_MoveTbl0[] = {
	//座標									回転率							拡大率					時間
	{ XMFLOAT3(3500.0f, 1100.0f, 0.0f),	XMFLOAT3(0.0f, 0.0f, 0.0f),	XMFLOAT3(1.0f, 1.0f, 1.0f),	100 },
};


static INTERPOLATION_DATA g_MoveTbl1[] = {
	//座標									回転率							拡大率							時間
	{ XMFLOAT3(3700.0f,1100.0f, 0.0f),XMFLOAT3(0.0f, 0.0f, 0.0f),	XMFLOAT3(1.0f, 1.0f, 1.0f),	60 },
};


static INTERPOLATION_DATA g_MoveTbl2[] = {
	//座標									回転率							拡大率							時間
	{ XMFLOAT3(5100.0f,1100.0f, 0.0f),	XMFLOAT3(0.0f, 0.0f, 0.0f),		XMFLOAT3(1.0f, 1.0f, 1.0f),	60 },
};
...
HRESULT InitTutorialEnemy()
{
	...
	// エネミー構造体の初期化
	g_EnemyCount = 0;
	for (int i = 0; i < TUTORIALENEMY_MAX; i++)
	{
		g_EnemyCount++;
		g_Enemy[i].use = TRUE;
		...
		if (g_Enemy[i].use == TRUE)
		{
			...
			if (g_Enemy[i].flushing != TRUE)
			{
				...
				if (g_Enemy[i].attacktime >= 100)
				{
					...
					{
						if (player[j].pos.x < g_Enemy[i].pos.x)
						{
							pos.x -= 100.0f;
							SetAttack(pos);
						}
