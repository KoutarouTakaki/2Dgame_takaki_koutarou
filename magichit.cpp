...
static char* g_TexturName[TEXTURE_MAX] = {
	"data/TEXTURE/magic_hit.png",
};

static BOOL		g_Load = FALSE;			// 初期化を行ったかのフラグ
static MAGICHIT	g_Hit[MAGICHIT_MAX];	// バレット構造体
			...
			// 当たり判定処理
			{
				...
				for (int j = 0; j < PLAYER_MAX; j++)
				{
					...
					if (player[j].use == TRUE)
					{
						...
						{
							BOOL ans = CollisionBB(g_Hit[i].pos, g_Hit[i].w * 0.5f, g_Hit[i].h * 0.5f,
